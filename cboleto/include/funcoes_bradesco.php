...
// | Desenvolvimen
to Boleto Bradesco: Ramon Soares						            |
// +----------------------------------------------------------------------+
*/
$codigobanco = "237";
...
function _fator_vencimento($data) {
  $data =explode("/",$data);
  $data_dias = _dateToDays($data[2], $data[1], $data[0]);
  ...
  if ($data_dias <= 2460728) {
      return(abs((_dateToDays("1997","10","07")) - $data_dias));
  } else {
      return(1000+abs((_dateToDays("2025","02","22")) - $data_dias));
  }
}
